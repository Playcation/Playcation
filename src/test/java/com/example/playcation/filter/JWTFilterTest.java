...
class JWTFilterTest {
  ...
  void shouldRefreshAccessTokenWhenExpired() throws Exception {
    ...
    when(jwtUtil.getCategory(validRefreshToken)).thenReturn(TokenSettings.REFRESH_TOKEN_CATEGORY);
    when(tokenService.createNewToken(any())).thenReturn(new String[]{newAccessToken, newRefreshToken});
    when(jwtUtil.createCookie(any(), any())).thenReturn(new Cookie(newAccessToken, newRefreshToken));
